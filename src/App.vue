<template>
  <div id="app">
    <transition :name="$store.state.app.slideName" mode="out-in">
    <router-view v-if="!$route.meta.keepAlive"></router-view>
    </transition>
    <!-- 缓存首页 -->
    <keep-alive>
        <router-view v-if="$route.meta.keepAlive"></router-view>
    </keep-alive>

    <div v-if="openLoading" id="loading-wrapper" class="loading-wrapper" style="/* display: none; */">
      <img class="loading-img" src="data:image/jpeg;base64,">
    </div>
  </div>
</template>

<script>
import {mapGetters} from 'vuex'

export default {
  name: 'App',
  data() {
    return {
      transitionName: ''
    }
  },
  computed: {
    ...mapGetters({
      timeDate: 'timeDate',
      openLoading: 'openLoading'
    })
  },
  mounted() {
    // this.timeDate = this.$store.state.app.slideName
    console.log(this.timeDate)
  }
}
</script>

<style>
  html,body{
    height: 100%;
    margin: 0;
    padding: 0;
    overflow-x:hidden;
  }
  #app {
    width: 100%;
    height: 100%;
    font-family: 'Avenir', Helvetica, Arial, sans-serif;
    -webkit-font-smoothing: antialiased;
    -moz-osx-font-smoothing: grayscale;
  }
  #app >div{
    min-height: 100%;
  }
  .loading-wrapper {
    position: fixed;
    left: 0;
    top: 0;
    right: 0;
    bottom: 0;
    z-index: 10000;
    background: rgba(255,255,255,.6);
    opacity: 0.3;
  }
  .loading-wrapper .loading-img {
    position: absolute;
    width: 30px;
    height: 30px;
    top: 45%;
    left: 50%;
    margin-top: -15px;
    margin-left: -15px;
    text-align: center;
  }
  /*切换路由的动画*/
  .right-enter-active,
  .fade-leave-active {
    transition: all .3s ease;
  }

  /*.fade-enter,*/
  .right-leave-active {
    opacity: 1;
    transform: translateX(100%);
  }

  .right-enter {
    opacity: 0;
    transform: translateX(-100%);
  }

  /*从底部出现*/
  .bottom-enter-active,
  .fade-leave-active {
    transition: all .3s ease;
  }

  /*.fade-enter,*/
  .bottom-leave-active {
    opacity: 1;
    transform: translateY(-30%);
  }

  .bottom-enter {
    opacity: 0;
    transform: translateY(30%);
  }


  /*返回路由的动画*/
  .left-enter-active,
  .fade-leave-active {
    transition: all .3s ease;
  }

  /*.fade-enter,*/
  .left-leave-active {
    opacity: 1;
    transform: translateX(-100%);
  }

  .left-enter {
    opacity: 0;
    transform: translateX(100%);
  }
</style>
